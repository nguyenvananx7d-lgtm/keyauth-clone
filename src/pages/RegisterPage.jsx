...
export default function RegisterPage() {
  ...
  const CheckItem = ({ ok, label }) => (
    <div className={`${styles.checkItem} ${ok ? styles.checkOk : ''}`}>
      <span>{ok ? '✓' : '○'}</span> {label}
    </div>
  )

  const SocialBtn = ({ provider, label, children }) => (
    <button
      className={styles.socialBtn}
      onClick={() => handleSocial(provider)}
      disabled={!!socialLoading}
    >
      {socialLoading === provider ? <span className={styles.socialLoader} /> : children}
      {label}
    </button>
  )
      ...
      <div className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.passwordChecks}>
            <CheckItem ok={checks.length} label="12+ ký tự" />
            <CheckItem ok={checks.upper} label="Chữ hoa" />
            <CheckItem ok={checks.number} label="Con số" />
            <CheckItem ok={checks.symbol} label="Biểu tượng" />
          </div>
        ...
        <div className={styles.socialBtns}>
          <SocialBtn provider="google" label="Google">
          ...
          <SocialBtn provider="github" label="GitHub">
          ...
          <SocialBtn provider="discord" label="Discord">
